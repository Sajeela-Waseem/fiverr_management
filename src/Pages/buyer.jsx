          ...
          {gigs.map((gig, index) => (
            <div key={index} className="relative bg-white rounded-xl shadow-md overflow-hidden transition-transform hover:scale-105 flex flex-col">
              ...
              <div className="p-2 flex flex-col flex-grow">
                          <div
  onClick={() => navigate(`/seller/${gig.sellerUid}`)}
  className="flex items-center pb-2 gap-3 mt-3 cursor-pointer group"
>
  <img
    src={
      gig.sellerImage && gig.sellerImage.trim() !== ""
        ? gig.sellerImage
        : `https://ui-avatars.com/api/?name=${encodeURIComponent(gig.sellerName || "Seller")}`
    }
    onError={(e) => {
      e.target.onerror = null;
      e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(gig.sellerName || "Seller")}`;
    }}
    alt={gig.sellerName}
